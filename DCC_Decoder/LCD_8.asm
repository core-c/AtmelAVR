;todo Ron: na een LCD_NextLine de volgende regel leeg maken (per SW instelbaar) (zoals wordwrap)
;********************************************************************************
;***
...
;CChar4:.dw	0b0000000000000000,0b0000000000000000,0b0000000000000000,0b0000000000000000

;---------------------------------------
;--- Teksten in programma(/flash)-geheugen
;--- Het SplashScreen
SplashText1: .db "   AVR-Team UJE   ", EOS_Symbol, 0
SplashText2: .db "  LCD  I2C-Slave  ", EOS_Symbol, 0
;--------------------------------------/
;--- InfoScreen
InfoText0: .db " Firmware v0.22-RD  ", EOS_Symbol, 0
InfoText1: .db "   TWI_Slave_02.asm ", EOS_Symbol, 0
InfoText2: .db "   LCD_8.asm        ", EOS_Symbol, 0
InfoText3: .db "   C:3222 D:62 E:32 ", EOS_Symbol, 0
;--------------------------------------/
...
DisplayMode_From_Flags:
	...
	breq	TestCursorOn				; Display vlag niet aan?
	ori		R16, DISPLAY_ON
TestCursorOn:
	; Cursor aan/uit
	mov		R17, LCD_Flags
	andi	R17, (1<<Flag_Cursor)
	breq	TestCursorBlock				; Cursor vlag niet aan?
	ori		R16, CURSOR_ON
TestCursorBlock:
	; Cursor Block/Streep
	mov		R17, LCD_Flags
	andi	R17, (1<<Flag_CursorBlock)
	breq	SetDisplayMode				; BlockCursor vlag niet aan?
...
LCD_Dec:
	...
	; Z is nu < 10..
	mov		R16, ZL
	rcall	LCD_HexNibble				; 5e cijfer afbeelden
	pop		R2
	pop		YH
	pop		YL
	pop		R16
...
Display_Digit:
	...
	brne	Digit_out					; ..niet meer nodig, "gewoon" afbeelden!
	cpi		YL, 0
...
Digit_out:
	...
	mov		R2, R16						;
	mov		R16, YL						; Y is altijd kleiner dan 10..
	rcall	LCD_HexNibble				; 1e cijfer afbeelden
Done_Display_Digit:
	pop		R16
	ret
;--------------------------------------/
