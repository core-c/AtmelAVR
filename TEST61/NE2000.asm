...
.equ NE2K_RESET		= NE2K_BASE + 0x18 ;NE2K_BASE + 0x1f
...
.equ NE2K_CR_PAGE3			= 0xC0		; select register page 3
...
;.equ NE2K_START_PAGE	= 0x40			; receive buffer ring from
;.equ NE2K_STOP_PAGE		= 0x80			; 0x4000-0x7fff
;--------------------------------------/
...
;control naar ISA-slot
.equ NE2K_ISA_PORT		= PORTA
.equ NE2K_CTRL_PORT		= NE2K_ISA_PORT
.equ NE2K_ISA_IOR_PIN	= PA5
.equ NE2K_ISA_IOW_PIN	= PA6
.equ NE2K_ISA_RESET_PIN	= PA7
;maskers
.equ NE2K_ISA_ADDR		= 0b00011111	; adres bits
.equ NE2K_ISA_IOR		= (1<<NE2K_ISA_IOR_PIN)
.equ NE2K_ISA_IOW		= (1<<NE2K_ISA_IOW_PIN)
.equ NE2K_ISA_RESET		= (1<<NE2K_ISA_RESET_PIN)
;; LED
;.equ LED_PORT		= PORTB
;.equ LED_PIN		= PB0
;.equ LED_DDR		= DDRB
...
;cbi		LED_PORT, LED_PIN			; LED uit
	...
	; stel het adres in (alleen de laagste 5 bits)
	mov r18, r16
	andi r18, NE2K_ISA_ADDR
	ori r18, NE2K_ISA_IOW | NE2K_ISA_IOR
	out NE2K_ADDR_OUT, r18				; IOW high
	; schrijf de data
	out NE2K_DATA_OUT, r17
	; Data poort als output instellen
	ldi r18, PORT_AS_OUTPUT
	out NE2K_DATA_DDR, r18

	; Een IOW puls geven.
	; hiermee neemt de NE2000 de aangeboden waarden over.
	nop									; duur = +- 136 ns @ 7.372800MHz
	cbi NE2K_CTRL_PORT, NE2K_ISA_IOW_PIN
	nop
	sbi NE2K_CTRL_PORT, NE2K_ISA_IOW_PIN
	nop

	ldi		r18, PORT_AS_INPUT
	out		NE2K_DATA_DDR, r18
	ldi		r18, INPUT_PORT_PULL
	out		NE2K_DATAPORT, r18
	pop r18
	ret
...
;cbi		LED_PORT, LED_PIN			; LED uit
	...
	ori r18, NE2K_ISA_IOW | NE2K_ISA_IOR
	out NE2K_ADDR_OUT, r18				; IOW high
	cbi NE2K_CTRL_PORT, NE2K_ISA_IOR_PIN; IOW low
	nop
	; lees de data
	in r17, NE2K_DATA_IN
	nop
	sbi NE2K_CTRL_PORT, NE2K_ISA_IOR_PIN; IOW high
	nop
	pop r18
	ret
...
NE2K_Hard_Reset:
	...
	sbi	NE2K_CTRL_PORT, NE2K_ISA_RESET_PIN
	rcall delay1ms						; 2 ms wachten zegt Ben, tenminste 800ns wachten zegt datasheet..
	rcall delay1ms						; 2 ms wachten zegt Ben, tenminste 800ns wachten zegt datasheet..
	cbi	NE2K_CTRL_PORT, NE2K_ISA_RESET_PIN
...
NE2K_Initialize:
	...
	; De data poort instellen als input
	; met interne pull-up weerstanden ingeschakeld.
	ldi		r18, PORT_AS_INPUT
	out		NE2K_DATA_DDR, r18
	ldi		r18, INPUT_PORT_PULL
	out		NE2K_DATAPORT, r18

	; poort A altijd als output instellen.
	; De laagste 5 bits van poort A zijn de adreslijnen naar het ISA-slot.
	; De controle signalen (IOR,IOW & RESET) hangen ook aan poort A (bits 7-5)
	; Deze controle signalen zijn ook altijd output.
	ldi		r18, PORT_AS_OUTPUT
...
NE2K_Read_MAC_EEPROM:
	...
	rcall NE2K_Write
	ldi r16, NE2K_RBCR0
	ldi r17, 32 						; intend to read 16*2 bytes
	rcall NE2K_Write
	ldi r16, NE2K_RBCR1
	ldi r17, 0
	rcall NE2K_Write
	...
	ldi r16, NE2K_CR
	ldi r17, NE2K_CR_PAGE0 | NE2K_CR_START | NE2K_CR_NODMA  ;(NE2K_CR_STOP ?!)
	rcall NE2K_Write
	pop r31
	pop r30
	pop r17
