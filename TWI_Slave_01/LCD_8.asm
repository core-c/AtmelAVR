...
.equ CopyViaSRAM		= 1					; boolean: 0 of 1
.equ WaitBusy_Method	= 0					; methode#: 0 (oudste), 1 (nieuwere manier) of 2 (laatste)
;------------------------------------------/
...
.equ Flag_BlockCursor	= 5	; bit 5: 1=Blok cursor,		0=Streep cursor
.equ Flag_Scrolling		= 4	; bit 4: 1=scrolling aan,	0=scrolling uit
.equ Flag_WordWrap		= 3	; bit 3: 1=word-wrap aan,	0=word-wrap uit
;--------------------------/
...
;           ;2,1 , ;4,3 , ;6,5 , ;8,7
;CChar0:	.dw	0b0000000000000000,0b0001000000010000,0b0001000000010000,0b0000000000000000
;CChar1:	.dw	0b0000000000000000,0b0001100000011000,0b0001100000011000,0b0000000000000000
;CChar2:	.dw	0b0000000000000000,0b0001110000011100,0b0001110000011100,0b0000000000000000
;CChar3:	.dw	0b0000000000000000,0b0001111000011110,0b0001111000011110,0b0000000000000000
;CChar4:	.dw	0b0000000000000000,0b0001111100011111,0b0001111100011111,0b0000000000000000

CChar0:	.dw	0b0000000000111100,0b0000000000000000,0b0000000000000000,0b0011110000000000
CChar1:	.dw	0b0011110000000000,0b0000000000000000,0b0000000000000000,0b0000000000000000
CChar2:	.dw	0b0000000000000000,0b0000000000111100,0b0000000000000000,0b0000000000000000
CChar3:	.dw	0b0000000000000000,0b0000000000000000,0b0011110000000000,0b0000000000000000
CChar4:	.dw	0b0000000000000000,0b0000000000000000,0b0000000000000000,0b0000000000111100

;---------------------------------------
;--- Teksten van het splashscreen
;--  in programma(/flash)-geheugen
SplashText0: .db " TWI_Slave_01.asm ", EOS_Symbol, 0
SplashText1: .db " PRG=No-INT Reply ", EOS_Symbol, 0
;--------------------------------------/
...
.MACRO LCD_Send;(register)
	LCD_E_Delay							; RS/RW setup time volgens datasheet 140ns
	LCD_CTRL_Enable1
    out		LCD_DATA_PORT, @0
	LCD_E_Delay
	LCD_CTRL_Enable0					; register temp wordt nu overgedragen
	LCD_E_Delay
	;LCD_CTRL_ToggleEnable
.ENDMACRO
...
;--- input: register Z = adres in EEPROM
...
.if WaitBusy_Method == 0
	WaitWhileFlag1:
		LCD_Receive R16						; Commando lezen van LCD (incl. busy flag)
		andi	R16, MASK_BUSYFlag			; busy flag filteren
		brne	WaitWhileFlag1

		;4us later is de addresscounter bijgewerkt..
		LCD_E_Delay
		LCD_E_Delay
		LCD_E_Delay
		LCD_E_Delay

.elif WaitBusy_Method == 1
	WaitWhileFlag1_:
		LCD_Receive R16						; Commando lezen van LCD (incl. busy flag)
		andi	R16, MASK_BUSYFlag			; busy flag filteren
		brne	WaitWhileFlag1_
		...
		LCD_E_Delay
		LCD_E_Delay
		;en dan de addresscounter pas inlezen..
		LCD_Receive R8						; Commando lezen van LCD (AC & BF)
		LCD_Receive R8						; Commando lezen van LCD (AC & BF)
		LCD_Receive R8						; Commando lezen van LCD (AC & BF)
		LCD_Receive R8						; Commando lezen van LCD (AC & BF)

.elif WaitBusy_Method == 2
		LCD_CTRL_Enable1					; E = 1
	WaitWhileFlag1__:
		LCD_E_Delay
		in		R16, LCD_DATA_IN
		andi	R16, MASK_BUSYFlag			; busy flag filteren
		brne	WaitWhileFlag1__
		LCD_CTRL_Enable0					; E = 0
		;4us later is de addresscounter bijgewerkt..
		LCD_E_Delay
		LCD_E_Delay
...
LCD_Read_Data:
    ...
    LCD_CTRL_Write						; R/W = 0 (schrijven)
	rcall	LCD_SetDATAPort_AS_OUTPUT
	; na een Read-opdracht tenminste 40us wachten..
	rcall	LCD_WaitWhileBusy			; wacht op de busy-flag
	rcall	delay100us
	ret
;--------------------------------------/
...
LCD_Command_:
	...
	; na een Commando tenminste 40us wachten..
	rcall	LCD_WaitWhileBusy			; wacht op de busy-flag
	rcall	delay100us
	ret
;--------------------------------------/
...
LCD_Data:
	...
	; na een Write-opdracht tenminste 40us wachten..
	rcall	LCD_WaitWhileBusy			; wacht op de busy-flag
	rcall	delay100us
	ret
;--------------------------------------/
...
LCD_Clear:
	...
	ldi		R16, CMD_Clear
    rcall	LCD_Command
	; na een Clear commando tenminste 1.52ms wachten
	rcall	delay2ms
	pop		R16
	ret
;--------------------------------------/
...
LCD_Home:
    ...
    rcall	LCD_Command
	; na een Home commando tenminste 1.52ms wachten
	rcall	delay2ms
	pop		R16
	ret
;--------------------------------------/
...
LCD_SetCharGenAddr:
    ...
    rcall	LCD_Command
	; Na een CGRAM/DDRAM wissel tenminste 120us wachten
	rcall	delay100us
	; ..Een 40us delay zit al in LCD_Command
	ret
;--------------------------------------/
...
LCD_Init:
	...
	rcall	delay100ms					; tenminste 40 milli-seconde pauze na het inschakelen van de power (2.7V, 15ms bij 4.5V)
...
LCD_Initialize:
	...
	ldi		R16, (1<<Flag_Display | 1<<Flag_Cursor | 1<<Flag_BlockCursor | 1<<Flag_Scrolling | 0<<Flag_WordWrap)
	mov		LCD_Flags, R16
	;
	pop		R17
	pop		R16
...
GoToRow1:
	...
	rcall	LCD_SetCursorXY
	rcall	LCD_ClearLineFromXY			; 1e regel legen
...
LCD_WordWrap:
	...
	;(de cursor staat nu op kolom(0) van de nieuwe regel. Het teken(R16) is nog niet afgebeeld)
	...
	; ..en scan naar een spatie als sein voor het einde van het woord.
...
WordEnd:
	...
	rcall	LCD_Read_Data				; lees het teken
	cpi		R16, ' '					; spatie?..
	breq	EndOfWord
	cpi		R17, 0						; zijn we vooraan de regel?
	breq	Done_Word					; geen wrapping uitvoeren, de regel staat vol met 1 woord.
	rjmp	WordEnd
EndOfWord:
...
Done_WordWrap:
	...
	; de LinePrint routine niet verder moet wortden uitgevoerd. (WordWrap routine heeft alles afgehandeld)
	pop		R9
	pop		R8
	pop		R7
	pop		R21
...
printChar:
	...
	breq	Exit_LinePrintChar			; nee: klaar..
...
SetNewLine:
	rcall	LCD_SetCursorXY				; de cursor-positie instellen
	rcall	LCD_ClearLineFromXY			; de "nieuwe" regel legen vanaf regel(R18):kolom(R17)..
CheckWordWrap:
	; het laatste teken onthouden dat een scroll veroorzaakt,
	; of dat een wordwrap kan veroorzaken.
...
;		rcall	LCD_GetDisplayAddr
	...
	CopyLoop:
		rcall	LCD_Read_Data				; teken lezen op cursor-positie
...
delay40us:	; @4.000000MHz = 160 cycles
	push	R16
	; een rcall instructie neemt 3 cycles in beslag	|	  1*3 =   3 +
	ldi		R16, 51						; 1 cycle	|	  1*1 =   1 +
innerloop40us:							;			|
	dec		R16							; 1 cycle	|	 51*1 =  51 +
	brne	innerloop40us				; 2 cycles	>	 50*2 = 100 +	;50 x wordt de branche genomen (springen = 2 cycles)
...
delay50us:	; @4.000000MHz = 200 cycles
	push	R16
	; een rcall instructie neemt 3 cycles in beslag	|	  1*3 =   3 +
	ldi		R16, 64						; 1 cycle	|	  1*1 =   1 +
innerloop50us:							;			|
	dec		R16							; 1 cycle	|	 64*1 =  64 +
	brne	innerloop50us				; 2 cycles	>	 63*2 = 126 +	;63 x wordt de branche genomen (springen = 2 cycles)
										; 1 cycle	|	  1*1 =   1 +	;bij de laatste branche wordt niet gesprongen (1 cycle)
	nop									; 1 cycle   |     1*1 =   1 +
	pop		R16
	ret									; 4 cycles	|	  1*4 =   4
...
;										;				-------------
;										;				        738 clockcycles
delay100us:	; @4.000000MHz = 400 cycles
	push	R16
	; een rcall instructie neemt 3 cycles in beslag	|	  1*3 =   3 +
...
innerloop4_1ms_:						;			|							|	 16400 cycles
	...
	brne	innerloop4_1ms_				; 2 cycles	>	   91*2 =    182 +		|
										; 1 cycle	|	    1*1 =      1 +		|
	nop									; 1 cycle	|	    1*1 =	   1 +		|
	pop		R16
	ret									; 4 cycles	|	    1*4 =      4		|
...
delay100ms:	;@4MHz						; deze delay timing is niet precies 100ms (ongeveer)
	push	R16
	ldi		R16,100
delay100ms_:
	rcall	delay1ms
