...
;---    ldi R16, 13                  ; De te schrijven byte met waarde 13
;---    StoreByteSRAMInBuffer R16, KeyBuffer, KeyInput, KeysInBuffer, KeyBufferSize
;---
;---   De code om een byte te lezen uit de buffer en in register R16 te plaatsen:
;---    LoadByteSRAMFromBuffer R16, KeyBuffer, KeyOutput, KeysInBuffer, KeyBufferSize
;---
...
; zet de zero-flag als de buffer leeg is.
; clear de zero-flag als de buffer gevuld is met tenminste 1 byte.
;
...
.MACRO IsBufferEmpty;(aAddressBytesInBuffer)
	...
	cpi		R25, 0
	sez							; de zero-flag zetten
	breq	_41					; de buffer is leeg
	clz							; De zero-flag wissen als buffer niet leeg is
_41:
	pop		R25
.ENDMACRO
...
;---------------------------------------------------------------------------------------
; Peek een byte uit de buffer (zonder bufferpointers te veranderen)
; indien mogelijk: carry gewist, anders wordt de carry gezet.
;                  aRegister (<>R25) bevat de gepeekte byte.
.MACRO PeekByteSRAMFromBuffer;(aRegister, aAddressOutput, aAddressBytesInBuffer
	push	R25
	push	ZL
	push	ZH
	; test of er bytes in de buffer staan om te lezen.
	; zoniet? dan de carry-flag zetten en macro beëindigen.
	lds		R25, @2				; R25 = aAddressBytesInBuffer^
	cpi		R25, 0
	sec
	breq	Done_Peek			; de buffer is leeg

	; de byte uit de buffer lezen
	; zonder bufferpointers te veranderen.
	lds		ZL, @1				; low(aAddressOutput^)
	lds		ZH, @1+1			; high(aAddressOutput^)
