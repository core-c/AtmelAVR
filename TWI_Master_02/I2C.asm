...
.equ MasterAddress		= $01			; master-adres moet <>0 & <8 zijn
.equ SlaveAddress_LCD	= $30			; slaves-adressen vanaf 8 tot 127
...
;--------------------------------------\
.equ PostScaler_ = 1					;   1 ->   1x1ms=1ms			per halve bitcycle op de bus => 2ms/bitcycle
.equ PostScaler = 1 ;200					; 200     200x5us=1000us=1ms
;---------------------------------------
.DSEG
...
;------------------------------------------\
;--- input: R22 = aantal te schrijven bytes
;---        RS232_IN_Buffer bevat de te verzenden data-bytes
TWI_WriteToSlave:
	push	R16
	sbi		LED_PORT, LED_TWI				; "TWI Activity" LED aan

NextDataByteOUT:
	rcall	LoadByteFrom_RS232_IN_Buffer	; laad een data-byte

	push	TWI_Address
	rcall	TWI_init
	lsl		TWI_Address						; Slave-adres in bit7-1 plaatsen
	ori		TWI_Address, TWI_Flag_Write		; Read/not-write in bit0
	rcall	TWI_start						; Zend START & ADRES
	mov		TWI_Data, R16
	rcall	TWI_do_transfer					; Overdracht starten
	rcall	TWI_stop						; STOP
	pop		TWI_Address

	tst		R22								; geen overflow aub.
	breq	StopWriting						; ..
	dec		R22								; aantal data-bytes te gaan
	brne	NextDataByteOUT
StopWriting:
...
;--------------------------------------\
SlaveFoundString:
	.db 13,10, "I2C-Slave(s) @ ", EOS_Character
;---------------------------------------
TWI_FindSlaves:
