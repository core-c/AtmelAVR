...
.equ flagINS		= (1<<0)			; Tekens invoegen?
; SPECIAL flags bit toekenningen
.equ flagF1			= (1<<7)			; F1 toggle-status (wissel afbeelden ASCII/scancodes
.equ flagAltNumber	= (1<<6)			; Alt - KP nummer invoeren?
...
KB_ScancodeToASCII:
	...
	LoadByteSRAM temp, ASCIIKey
	ldi		ZH, HIGH(ScanCodesKP1)
	ldi		ZL, LOW(ScanCodesKP1)
	rcall	ReadTableEEPROM
...
_isExtended:
	ldi		temp, flagExtended	; is de Extended Flag gezet?
	rcall	ReadFlags_ASCII		;
	brne	_YesExtended		; Flag niet gezet? dan jump naar _SpecialKeys1
	rjmp	_SpecialKeys1
_YesExtended:
	; Er is een toets met een extended scancode losgelaten
	LoadByteSRAM temp, ASCIIKey
...
_keyAltR: ;extended scancode
	...
	rjmp	_SetFlagAlt
_KPDiv:
	cpi		temp, scKPDiv ;extended scancode
	brne	_ArrowUp
	StoreIByteSRAM ASCIIKey, $FD	; LCD Gedeeld-door teken
	rjmp	Done_DecodeKey
...
_Home:
	...
	rjmp	ForgetKey
_End:
	cpi		temp, scEnd ;extended scancode
	brne	_KPEnter
	rcall	LCD_LineEnd
	rjmp	ForgetKey
_KPEnter:
	...
	rjmp	ForgetKey
_INS:
	cpi		temp, scINS ;extended scancode
	brne	_DEL
	ldi		temp, flagINS		; de INSert Flag...
	rcall	ToggleFlags_ASCII	; ... flippen in ASCIIFlags
	; LCD cursor-style wisselen
	ldi		temp, flagINS		; is de INSert Flag gezet?...
	rcall	ReadFlags_ASCII		; ... in ASCIIFlags
	breq	_INSlcdOverwrite
	ldi		temp, (DISPLAY_ON | CURSOR_ON | CURSOR_BLINK_OFF)
	rjmp	_INSlcdCursorShape
  _INSlcdOverwrite:
...
_CLS:							; De beeldinhoud van het LCD wissen & cursor op begin 1e regel
	...
	rcall	LCD_Home
	rjmp	ForgetKey
	;--- IN COMBINATIE MET Ctrl
_CtrlCombination:
	ldi		temp, flagCtrl		; is de Ctrl Flag gezet?...
	rcall	ReadFlags_ASCII		; ... in ASCIIFlags
	LoadByteSRAM temp, ASCIIKey
	brne	_CtrlG				; Flag gezet? dan jump naar _CtrlG
	rjmp	_Convert ;ForgetKey
_CtrlG:
	cpi		temp, scG
	brne	_CtrlX
	StoreIByteSRAM ASCIIKey, $DF	; LCD graden teken
	rjmp	Done_DecodeKey
_CtrlX:
	cpi		temp, scX
	brne	_CtrlP
	StoreIByteSRAM ASCIIKey, $EB	; LCD machtsverheffen teken
	rjmp	Done_DecodeKey
_CtrlP:
	cpi		temp, scP
	brne	_CtrlO
	StoreIByteSRAM ASCIIKey, $F7	; LCD Pi teken
	rjmp	Done_DecodeKey
_CtrlO:
	cpi		temp, scO
	brne	_CtrlU
	StoreIByteSRAM ASCIIKey, $F4	; LCD Ohm teken
	rjmp	Done_DecodeKey
_CtrlU:
	cpi		temp, scU
	brne	_CtrlB
	StoreIByteSRAM ASCIIKey, $E4	; LCD Mirco teken
	rjmp	Done_DecodeKey
_CtrlB:
	cpi		temp, scB
	brne	_CtrlA
	StoreIByteSRAM ASCIIKey, $E2	; LCD Beta teken
	rjmp	Done_DecodeKey
_CtrlA:
	cpi		temp, scA
	brne	_Ctrl1
	StoreIByteSRAM ASCIIKey, $E0	; LCD Alpha teken
	rjmp	Done_DecodeKey
_Ctrl1:
	cpi		temp, sc1
	brne	_CtrlDot
	StoreIByteSRAM ASCIIKey, $E9	; LCD TotDeMacht-1 teken
	rjmp	Done_DecodeKey
_CtrlDot:
	cpi		temp, scDot
	brne	_CtrlW
	StoreIByteSRAM ASCIIKey, $A5	; LCD Punt-in-het-midden teken
	rjmp	Done_DecodeKey
_CtrlW:
	cpi		temp, scW
	brne	ForgetKey ;_Convert		; geen aparte scancode terwijl Ctrl ingedrukt? dan vergeten..
	StoreIByteSRAM ASCIIKey, $E8	; LCD Worteltrekken teken
	rjmp	Done_DecodeKey
	;-------------------------------------------/
_SetFlagExtended:
	ldi		temp, flagExtended	; de Extended Flag...
...
;------------------------------------------
;--- LCD GetINS
;--- output: zet de zero-flag als de 
;---         INSert-flag is gezet in ASCIIFlags
;---         , anders de zero-flag wissen.
;------------------------------------------
LCD_GetINS:
	push	temp
	ldi		temp, flagINS		; is de INSert Flag gezet?...
