...
;Affected:	none
; het Z-register blijft "onaangetast" (waarde blijft gelijk)
...
;---------------------------------------------------------------------------------------
; store een Immediate byte-waarde in SRAM
;Affected:	mRegH
.MACRO StoreIByteSRAM;(aAddress, aByte)
;	push	mRegH
	ldi		mRegH, @1			; mRegH = aByte
	sts		@0, mRegH			; aAddress^ = mRegH
;	pop		mRegH
.ENDMACRO
...
; store een Immediate word-waarde in SRAM
;Affected:	mRegL
;			mRegH
.MACRO StoreIWordSRAM;(aAddress, aWord)
;	push	mRegL
;	push	mRegH
	ldi		mRegL, low(@1)		; mRegL = low(aWord)
	ldi		mRegH, high(@1)		; mRegH = high(aWord)
	sts		@0, mRegL			; aAddress^ = mRegL
	sts		@0+1, mRegH			; (aAddress+1)^ = mRegH
;	pop		mRegH
;	pop		mRegL
.ENDMACRO
...
;	push	mRegH
	;
	lds		mRegH, @0			; mRegH = aAddressBytesInBuffer^
	cpi		mRegH, 0
	clc
...
;	push	ZL
	...
	lds		mRegH, @3			; mRegH = aAddressBytesInBuffer^
	cpi		mRegH, @4			; mRegH ? aBufferSize
...
_2:; (NIET voorbij EOB)
	...
	lds		mRegH, @3			; mRegH = aAddressBytesInBuffer^
	inc		mRegH
	sts		@3, mRegH			; aAddressBytesInBuffer^ = mRegH
...
;	push	ZL
	...
	lds		mRegH, @3			; aAddressBytesInBuffer
	cpi		mRegH, 0
	sec
	...
	ld		@0, Z+				; aRegister, Z met 1 verhogen, zodat deze naar de volgende te lezen byte wijst.
