...
;***  Timer0 regelt de timing voor DelayedActions om de 1/10 seconde.
;***
...
;**
;***  Timer0 prescale is ingesteld op CK/64 => 7372800/64=115200Hz. Met een
;***  PostScaler van 45 komt de doelfrequentie op 115200/45=2560Hz. De Timer0-
;***  Overflow Interrupt komt om elke 256 timer-ticks; Dus dan 10 keer per seconde.
;***  We hebben dan met Timer0 een super-strakke timing op 10Hz.
;***
...
; De PostScaler
.equ vertraging	= 45					; om de 'vertraging'-aantal Timer0-Overflow's speciale code uitvoeren
...
.org 0x0000								; Het begin van de vector-tabel op adres 0 van het geheugen
	...
	rjmp INT0_Interrupt					; External Interrupt 0
	reti								; External Interrupt 1
	reti;rjmp Timer2_Compare			; Timer2 compare match
	reti;rjmp Timer2_Overflow			; Timer2 overflow
	reti								; Timer1 Input Capture
	reti;rjmp Timer1A_Compare			; Timer1 Output Compare A
	reti;rjmp Timer1B_Compare			; Timer1 Output Compare B
	reti;rjmp Timer1_Overflow			; Timer1 Overflow
	rjmp Timer0_Overflow				; Timer0 Overflow
	reti								; SPI
	rjmp UART_Rx_Interrupt				; UART Receive Complete
	reti								; UART Data Register Empty
	reti;rjmp UART_Tx_Interrupt			; UART Transmit Complete
...
RESET:
	...
	ldi		LED2_Brightness, $00
	ldi		LED1A_Brightness, $00
	ldi		LED1B_Brightness, $00
	; de (software)PostScaler vertraging instellen
	...
	;
	; Timer0 wordt gebruikt voor de PostScaled DelayedActions;
	; Het LED-looplicht en 
	; de UART-Transmissies naar de PC van de ADC-samples.
;	ldi		temp, (0<<OCIE1A | 0<<OCIE1B | 0<<TOIE1 | 0<<OCIE2 | 1<<TOIE2 | 0<<TOIE0) ; alleen overflow T2
	ldi		temp, (0<<OCIE1A | 0<<OCIE1B | 0<<TOIE1 | 0<<OCIE2 | 0<<TOIE2 | 1<<TOIE0) ; alleen overflow T0
	out		TIMSK, temp

	;--- De duty-cycles instellen van de 3 PWM-signalen
	clr		temp
	; Output Compare Register 1A (16-bits Timer1)
	out		OCR1AH, temp
	out		OCR1AL, LED1A_Brightness
	; Output Compare Register 1B (16-bits Timer1)
	out		OCR1BH, temp
	out		OCR1BL, LED1B_Brightness
	; Output Compare Register 2 (8-bits Timer2)
	out		OCR2, LED2_Brightness

	;--- Timer2-, Timer1- & Timer0-counter instellen
	;clr		temp ;!!!!! temp is nog leeg (==0)
	; Timer CouNTer 1
	out		TCNT1H, temp
	out		TCNT1L, temp
	; Timer CouNTer 2
...
;	ldi		temp, (1<<CS12 | 0<<CS11 | 1<<CS10);CK/1024
	...
	;--- De software-PreScaler
	; De clock van de Timer0 instellen op CK/64
	ldi		temp, (0<<CS02 | 1<<CS01 | 1<<CS00)		; CK/64
	out		TCCR0, temp
...
;---------------------------------------
;- De Timer0-Overflow Interrupt-Handler
;---------------------------------------
Timer0_Overflow:
;	push	temp
;	push	tempL
